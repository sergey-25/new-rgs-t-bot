    ...
    async function sendSupportRequestMessage(msg) {
      ...
      if (isFirstMenuCall) {
          ...
          const sendNextStep = async () => {
              ...
              const prompts = {
                  hospital_name: "Будь-ласка, вкажіть назву закладу або ЄДРПОУ закладу:",
                  recipient_name: "Будь-ласка, вкажіть як до Вас звертатися (ПІБ):",
                  recipient_phone: "Будь-ласка, вкажіть ваш контактий номер телефону:",
                  recipient_email: "Будь-ласка, вкажіть адресу електронної пошти телефону:",
                  problem: "Будь-ласка, детально опишіть проблему:"
              };

              await bot.sendMessage(chatId, prompts[step], {
                  force_reply: true
              });

              const response = await waitForReply(chatId);
              isFirstMenuCall = false; // Set this to false after the first menu call
          ...
          const handleCallback = async (response) => {
              supportRequest[steps[stepIndex]] = response;
              stepIndex++;
          ...
          bot.once("message", async (response) => {
              await handleCallback(response.text);
          });
      }
  }
